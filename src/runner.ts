...
export class PlyRunner {
    ...
    async runPlyees(plyees: string[], debug = false, runOptions?: object): Promise<void> {
        ...
        if (this.autoOpenOutput) this.outputChannel.show(true);
        ...
        return new Promise<void>((resolve) => {
            ...
            this.runningTestProcess = fork(childProcScript, [], {
                execPath: nodePath,
                execArgv,
            ...
            this.runningTestProcess.on('message', (message: any) => {
                if (typeof message === 'string') {
                    ...
                    if (message.type === 'flow') {
                        ...
                        if (
                            (message.type === 'test' && message.state === 'failed') ||
                            message.state === 'errored'
                        ) {
                            const msgTest = (message as TestEvent).message;
                            const testId =
                                typeof msgTest === 'string' ? msgTest : (msgTest as any).id;
                            const test = this.plyRoots.getTest(testId);
                            if (test) {
                                decorations.push({
                                    line: test.start || 0,
                                    message: `${message.state.toLocaleUpperCase()}: ${
                                        message.description
                                    }`
                                });
                            }
                        ...
                        if (message.type === 'test') {
                            if (message.state === 'running') {
                                ...
                                if (runningTest) {
                                    this.diffState.updateDiffs(
                                        runningTest,
                                        (message as any).diffs || []
                                    );
                                }
     ...
     */
    private async checkMissingExpectedResults(
        testInfos: TestInfo[]
    ): Promise<ply.RunOptions | undefined> {
        ...
        for (const testInfo of testInfos) {
            ...
            if (suite) {
                if (!suite.skip) {
                    ...
                    if (
                        !expectedExists &&
                        !suitesWithMissingResults.find((s) => s.path === suite?.path)
                    ) {
        ...
        if (suitesWithMissingResults.length > 0) {
            ...
            if (firstMissingExpected.location.isChildOf(this.workspaceFolder.uri.fsPath)) {
                firstMissing = firstMissingExpected.location.relativeTo(
            ...
            items.push(noVerify);
            const addToSkip = {
                label: 'Add to "ply.skip" setting',
                description: 'exclude suite from bulk runs'
            };
            const createExpected = { label: 'Create expected result', description: 'from actual' };
            if (
                suitesWithMissingResults.reduce(
                    (accum, suite) => accum && !suite.runtime.results.expected.location.isUrl,
                    true
                )
            ) {
                    ...
                    for (const suite of suitesWithMissingResults) {
                        ...
                        if (typeof skip === 'string' && skip.trim().length > 0) {
                            ...
                            if (skip.startsWith('{')) {
                                ...
                                skip =
                                    skip.substring(0, closing) +
                                    ',' +
                                    suite.path +
                                    skip.substring(closing, skip.length);
                            } else {
                        ...
                        suite.skip = true;
                        vscode.workspace
                            .getConfiguration('ply', this.workspaceFolder.uri)
                            .update('skip', skip);
    ...
    private stringsOnly(env: { [envVar: string]: string | null | undefined }): {
