...
export function runTestsInFile(fileUri: string | undefined, testExplorer: PlyExplorer): void {
...
export function debugTestsInFile(fileUri: string | undefined, testExplorer: PlyExplorer): void {
    if (!fileUri && vscode.window.activeTextEditor) {
        fileUri = vscode.window.activeTextEditor.document.uri.toString();
    }
...
function findFileNodes(fileUri: string, searchNode: TreeNode): TreeNode[] {
    if (searchNode.fileUri) {
        ...
        return ([] as TreeNode[]).concat(
            ...searchNode.children.map((childNode) => findFileNodes(fileUri, childNode))
        );
    }
}

export function runTestAtCursor(testExplorer: PlyExplorer): void {
...
export function debugTestAtCursor(testExplorer: PlyExplorer): void {
    const editor = vscode.window.activeTextEditor;
    if (editor) {
        const nodes = findNodesLocatedAboveCursor(
            editor.document.uri.toString(),
...
function findNodesLocatedAboveCursor(
    fileUri: string,
    cursorLine: number,
    testExplorer: PlyExplorer
): TreeNode[] {
    let currentLine = -1;
...
export function groupSuitesByLabel(
    nodes: (TestSuiteInfo | TestInfo)[]
...
export function mergeSuiteInfos(suites: TestSuiteInfo[]): TestSuiteInfo {
    if (suites.length === 1) {
        ...
        return {
            ...
            children: ([] as (TestSuiteInfo | TestInfo)[]).concat(
                ...suites.map((suite) => suite.children)
            )
        };
    }
}
