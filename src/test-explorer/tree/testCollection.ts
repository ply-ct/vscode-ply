...
export class TestCollection {
    ...
    get autorunNode() {
        return this._autorunNode;
    }

    constructor(public readonly adapter: TestAdapter, public readonly explorer: PlyExplorer) {
        ...
        this.disposables.push(
            vscode.workspace.onDidChangeConfiguration((configChange) => {
                if (configChange.affectsConfiguration('ply.explorer.codeLens', workspaceUri)) {
        ...
        this.disposables.push(
            adapter.tests((testLoadEvent) => this.onTestLoadEvent(testLoadEvent))
        );
        this.disposables.push(
            adapter.testStates((testRunEvent) => this.onTestRunEvent(testRunEvent))
        );

        if (adapter.retire) {
            this.disposables.push(
                adapter.retire((retireEvent) => {
                    if (!this.rootSuite) return;

                    let nodes: TreeNode[];
                    if (retireEvent && retireEvent.tests) {
                        nodes = retireEvent.tests
    ...
    private onTestLoadEvent(testLoadEvent: TestLoadStartedEvent | TestLoadFinishedEvent): void {
        if (testLoadEvent.type === 'started') {
            ...
            if (this._autorunNode) {
                ...
                const autorunMemento = this.explorer.context.workspaceState.get<string>(
                    this.autorunMementoKey
                );
                if (autorunMemento) {
    ...
    private onTestRunEvent(
        testRunEvent: TestRunStartedEvent | TestRunFinishedEvent | TestSuiteEvent | TestEvent
    ): void {
        ...
        if (testRunEvent.type === 'started') {
            ...
            if (this.allRunningTests.has(testRunEvent.testRunId)) {
                for (const testNode of this.allRunningTests.get(testRunEvent.testRunId)!) {
                    if (
                        testNode.state.current === 'scheduled' ||
                        testNode.state.current === 'running'
                    ) {
            ...
            const suiteId =
                typeof testRunEvent.suite === 'string' ? testRunEvent.suite : testRunEvent.suite.id;
            ...
            if (testRunEvent.state === 'running') {
                if (
                    !suiteNode &&
                    this.runningSuite.has(testRunEvent.testRunId) &&
                    typeof testRunEvent.suite === 'object'
                ) {
                ...
                // testStateMessage.state === 'completed' || testStateMessage.state === 'errored'
            ...
            // testStateMessage.type === 'test'

            const testId =
                typeof testRunEvent.test === 'string' ? testRunEvent.test : testRunEvent.test.id;
            ...
            if (
                !testNode &&
                this.runningSuite.has(testRunEvent.testRunId) &&
                typeof testRunEvent.test === 'object'
            ) {
                const runningSuite = this.runningSuite.get(testRunEvent.testRunId)!;
            ...
            if (this.shouldShowCodeLens()) {
                for (const [file, fileLocatedNodes] of this.locatedNodes) {
                    ...
                    for (const [line, lineLocatedNodes] of fileLocatedNodes) {
                        ...
                        if (
                            lineLocatedNodes.some(
                                (node) => node.log !== undefined && node.log.length > 0
                            )
                        ) {
            ...
            for (const node of lineNodes) {
                if (node instanceof TestNode) {
                    for (const decoration of node.decorations) {
                        if (
                            position.line === decoration.line &&
                            position.character ===
                                document.lineAt(decoration.line).range.end.character
                        ) {
    ...
    private collectNodesByIdRec(node: TreeNode): void {
        if (!this.idCount.get(node.info.id)) {
            ...
            const errorMessage =
                'There are multiple tests with the same ID, Ply Explorer will not be able to show test results for these tests.';
            const errorDescription = '*** duplicate ID ***';

            if (node instanceof TestNode) {
                node.setCurrentState(
                    'duplicate',
                    errorMessage,
                    undefined,
                    errorDescription,
