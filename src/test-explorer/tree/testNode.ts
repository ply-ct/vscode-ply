...
import { TestInfo, TestDecoration } from '../../test-adapter/api/index';
import { TreeNode } from './treeNode';
import { NodeState, stateIcon, CurrentNodeState, defaultState, toUri } from './state';
...
export class TestNode implements TreeNode {
    ...
    get decorations(): TestDecoration[] {
    ...
    retireState(): void {
        if (
            this.state.current === 'passed' ||
            this.state.current === 'failed' ||
            this.state.current === 'skipped' ||
            this.state.current === 'errored'
        ) {
    ...
    resetState(): void {
        ...
        if (
            ...
            this.state.current !== 'always-skipped' &&
            this.state.current !== 'duplicate'
        ) {
            this._state.current = this.info.errored ? 'errored' : 'pending';
            this.sendStateNeeded = true;
        }
        ...
        if (
            this.state.previous !== 'pending' &&
            this.state.previous !== 'always-skipped' &&
            this.state.previous !== 'duplicate'
        ) {
    ...
    getTreeItem(): vscode.TreeItem {
        ...
        treeItem.iconPath = this.collection.explorer.iconPaths[stateIcon(this.state, this.info.id)];
        treeItem.contextValue =
            this.collection.adapter.debug && this.info.debuggable !== false
                ? this.fileUri
                    ? 'debuggableTestWithSource'
                    : 'debuggableTest'
                : this.fileUri
                ? 'testWithSource'
                : 'test';
        ...
        // set resourceUri for tree decoration
        treeItem.resourceUri = toUri(treeItem.id).with({ scheme: 'ply-explorer' });
        return treeItem;
    }
