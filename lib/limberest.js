  ...
  source.getMatches(options, function(err, matches) {
    ...
    matches.forEach(match => {
      ...
      if (options.localLocation) {
        ...
        testGroup.requests.forEach(request => {
          var storage = new Storage(options.localLocation + '/' + testGroup.name, 
                limbThis.getResourceName(request));
          if (storage.exists()) {
            var localRequest = storage.read();
            if (localRequest) {
              request = Object.assign({_local:true}, localRequest);
            }
...
Limberest.prototype.discardExpectedResult = function(options, groupName, request, result) {
  if (!options.localLocation)
    throw new Error('No localLocation specified in options');
  var resName = this.getResourceName(request.method, request.name, 'yaml');
  var storage = new Storage(options.localLocation + '/' + groupName, resName);
...
Limberest.prototype.saveExpectedResult = function(options, groupName, request, result) {
