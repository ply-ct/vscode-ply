...
// Does not merge local storage or set _location.
...
Limberest.prototype.loadValueObjs = function(options, callback) {
  ...
  source.getMatches(options, function(err, matches) {
    ...
    matches.forEach(match => {
      ...
      testVals = Object.assign({_origin:match.location, _location:match.location}, testVals);
      if (options.localLocation) {
        // merge from local
        var storage = new Storage(options.localLocation + '/' + testVals.name);
        if (storage.exists()) {
          testVals = storage.read();
          testVals._location = options.localLocation + '/' + testVals.name;
        }
      }
