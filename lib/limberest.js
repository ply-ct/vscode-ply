...
// Merges local storage if retrieved from remote.
// Group.file is a reconstruction incorporating local request changes. 
Limberest.prototype.loadGroups = function(options, callback) {
  ...
  source.getMatches(options, function(err, matches) {
    ...
    matches.forEach(match => {
      ...
      var reqGroup;
      var isPostman = postman.isCollection(obj);
      if (isPostman) {
      ...
      reqGroup.origin = match.origin;
      reqGroup.uiOrigin = match.uiOrigin;
      const fileName = reqGroup.name + options.extensions[0]; // TODO
      reqGroup.file = {
          name: fileName,
          path: options.path + '/' + fileName,
          origin: reqGroup.origin
      }
      
      if (options.localLocation) {
        // find local-only requests
        var grpStore = new Storage(options.localLocation + '/' + reqGroup.name);
        if (grpStore.exists()) {
          JSON.parse(grpStore.read()).forEach(localRequest => {
            reqGroup._local = true;
            reqGroup.requests.push(localRequest);
          });
        }
      ...
      reqGroup.file.contents = JSON.stringify(obj, null, 2); // TODO
...
Limberest.prototype.loadExpected = function(options, groupName, request, callback) {
  ...
  request.origin = options.expectedResultLocation + '/' + groupName + '.postman'; // TODO hardcoded .postman
  if (options.location.startsWith('https://github.com') && request.origin.startsWith('https://raw.githubusercontent.com')) {
    request.expectedUiOrigin = options.location + '/blob/' + options.branch + '/' + options.resultPath + '/' + groupName + 
        '/' + request.method + '_' + request.name + '.yaml';
  }
  if (options.localLocation) {
