...
Limberest.prototype.loadValues = function(location, callback) {
  var vals;
  this.loadFile(location, function(err, data) {
    if (!err) {
      try {
        if (!data)
          throw new Error('Values not found: ' + location);
...
Limberest.prototype.loadTestActual = function(groupName, method, testName, options, callback) {
...
Limberest.prototype.loadTestLog = function(groupName, method, testName, options, callback) {
...
Limberest.prototype.loadActual = function(path, resName, options, callback) {
  var storage = new Storage(new Options(options).options.resultLocation + '/' + path, resName + '.yaml');
  callback(null, storage.exists ? storage.read() : null);
};

Limberest.prototype.loadLog = function(path, resName, options, callback) {
  var storage = new Storage(new Options(options).options.logLocation + '/' + path, resName + '.log');
  callback(null, storage.exists ? storage.read() : null);
};
