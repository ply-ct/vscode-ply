...
var Case = exports.Case = function(name, options) {
  ...
  if (!options.expectedResultLocation)
    options.expectedResultLocation = options.resultLocation;
  if (!options.logLocation)
    options.logLocation = options.resultLocation;
  
  if (this.options.expectedResultLocation.startsWith('https://') || 
      this.options.expectedResultLocation.startsWith('http://')) {
    this.expectedResultUrl = this.options.expectedResultLocation;
    if (!this.expectedResultUrl.endsWith('/'))
      this.expectedResultUrl += '/';
    this.expectedResultUrl += require('sanitize-filename')(this.name, {replacement: '_'}) + '.yaml';
  }
  else {
    this.expectedResultStorage = new (storage.Storage)(this.options.expectedResultLocation, this.name + '.yaml');
  }
...
Case.prototype.verify = function(values) {
  ...
  try {
    if (this.expectedResultStorage) {
      ...
      var expected = this.expectedResultStorage.read().replace(/\r/g, ''); // in case someone edited on windows
      return this.verifyResult(expected, values);
    }
    else if (this.expectedResultUrl) {
      var request;
      if (typeof window === 'undefined')
        request = require('request').defaults({headers: {'User-Agent': 'limberest'}});
      else
        request = require('browser-request');
      var thisCase = this;
      request(this.expectedResultUrl, function(err, response, body) {
        if (response.statusCode != 200)
          throw new Error(response.statusCode + ': ' + codes[response.statusCode]);
        return thisCase.verifyResult(body.replace(/\r/g, ''), values);
      });
    }
  }
  catch (err) {
