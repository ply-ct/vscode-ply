...
var Case = exports.Case = function(name, options) {
  ...
  const logFile = this.options.logDir + '/' + baseName + '.log';
      ...
      try {
        ...
        if (!error) {
          ...
          if (this.options.overwrite) {
            ...
            mkdirsSync(this.options.caseDir);
...
Case.prototype.verify = function(values) {
  ...
  if (!fs.existsSync(expectedFile))
    throw new NoExpectedResult('Expected result file not found: ' + expectedFile);
...
Case.prototype.getActualResultFile = function() {
  var file = this.options.resultDir;
  return this.options.resultDir + '/' + this.getBaseFileName() + '.yaml';
}

Case.prototype.getExpectedResultFile = function() {
  return this.options.caseDir + '/' + this.getBaseFileName() + '.yaml';
}
